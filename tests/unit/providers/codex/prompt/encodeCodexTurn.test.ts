...
describe('encodeCodexTurn', () => {
  ...
  it('should include ordered external references in the prompt', () => {
    const request: ChatTurnRequest = {
      text: 'Use these references',
      externalContextPaths: ['/vault/folder-b', '/vault/file-a.md'],
    };
    const result = encodeCodexTurn(request);

    expect(result.prompt).toContain('<external_references>');
    expect(result.prompt).toContain('1. /vault/folder-b');
    expect(result.prompt).toContain('2. /vault/file-a.md');
  });
