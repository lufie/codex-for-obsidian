...
describe('encodeClaudeTurn', () => {
  ...
  it('should append ordered external references to the prompt content', () => {
    const request: ChatTurnRequest = {
      text: 'summarize these references',
      externalContextPaths: ['/vault/beta', '/vault/alpha/file.md'],
    };
    const result = encodeClaudeTurn(request, mcpManager);

    expect(result.persistedContent).toContain('<external_references>');
    expect(result.persistedContent).toContain('1. /vault/beta');
    expect(result.persistedContent).toContain('2. /vault/alpha/file.md');
  });
