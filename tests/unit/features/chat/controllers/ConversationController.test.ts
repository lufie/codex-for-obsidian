...
describe('ConversationController - Persistent External Context Paths', () => {
  ...
  let mockExternalFileAttachmentManager: any;
  ...
  beforeEach(() => {
    ...
    mockExternalFileAttachmentManager = {
      getAttachedPaths: jest.fn().mockReturnValue([]),
      clearAttachments: jest.fn(),
      setAttachedPaths: jest.fn().mockResolvedValue(undefined),
    };
    deps = createMockDeps({
      getExternalContextSelector: () => mockExternalContextSelector,
      getExternalFileAttachmentManager: () => mockExternalFileAttachmentManager,
    });
  ...
  describe('createNew', () => {
    it('should call clearExternalContexts with persistent paths from settings', async () => {
      ...
      expect(mockExternalFileAttachmentManager.clearAttachments).toHaveBeenCalled();
    });
  ...
  describe('loadActive', () => {
    ...
    it('should restore saved paths for conversation with messages (msg>0)', async () => {
      ...
      deps.plugin.getConversationById = jest.fn().mockResolvedValue({
        ...
        externalAttachmentPaths: ['/attached/a', '/attached/b'],
      });

      await controller.loadActive();

      expect(mockExternalContextSelector.setExternalContexts).toHaveBeenCalledWith(['/saved/path']);
      expect(mockExternalFileAttachmentManager.setAttachedPaths).toHaveBeenCalledWith([
        '/attached/a',
        '/attached/b',
      ]);
  ...
  describe('switchTo', () => {
    ...
    it('should use persistent paths when switching to empty conversation (msg=0)', async () => {
      (deps.plugin.switchConversation as jest.Mock).mockResolvedValue({
        ...
        externalAttachmentPaths: ['/old/attached/path'],
      });
