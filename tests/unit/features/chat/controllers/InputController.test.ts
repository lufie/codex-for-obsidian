...
describe('InputController - Message Queue', () => {
  ...
  describe('External context in query', () => {
    ...
    it('should prioritize ordered dragged references ahead of external context roots', async () => {
      deps = createSendableDeps({
        getExternalContextSelector: () => ({
          getExternalContexts: () => ['/context/root', '/shared/path'],
          addExternalContext: jest.fn(),
        }) as any,
        getExternalFileAttachmentManager: () => ({
          getAttachedPaths: () => ['/ref/b', '/shared/path', '/ref/a'],
        }) as any,
      });
      ...
      inputEl = deps.getInputEl() as ReturnType<typeof createMockInputEl>;
      inputEl.value = 'test [[ref:%2Fref%2Fb]] [[ref:%2Fshared%2Fpath]] [[ref:%2Fref%2Fa]] message';
      controller = new InputController(deps);

      await controller.sendMessage();

      const prepareTurnCall = ((deps as any).mockAgentService.prepareTurn as jest.Mock).mock.calls[0];
      expect(prepareTurnCall[0].externalContextPaths).toEqual([
        '/ref/b',
        '/shared/path',
        '/ref/a',
        '/context/root',
