...
function createContextRow(browserIndicator: HTMLElement | null): HTMLElement {
  ...
  const externalFilesChips = { style: { display: 'none' } };
  const lookup = new Map<string, unknown>([
    ...
    ['.claudian-external-files-chips', externalFilesChips],
  ]);
...
describe('updateContextRowHasContent', () => {
  ...
  it('treats external file chips as visible content when display is flex', () => {
    const contextRowEl = createContextRow(null);
    const externalFilesChips = contextRowEl.querySelector('.claudian-external-files-chips') as HTMLElement;
    externalFilesChips.style.display = 'flex';
