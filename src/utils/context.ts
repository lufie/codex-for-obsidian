 ...
 * context_files, canvas_selection, browser_selection, external_references, external_reference
 */
export const XML_CONTEXT_PATTERN = /\n\n<(?:current_note|editor_selection|editor_cursor|context_files|canvas_selection|browser_selection|external_references|external_reference)[\s>]/;
...
export function extractUserQuery(prompt: string): string {
  ...
  return prompt
    ...
    .replace(/<external_references>[\s\S]*?<\/external_references>\s*/g, '')
    .replace(/<external_reference[\s\S]*?<\/external_reference>\s*/g, '')
    .trim();
}
