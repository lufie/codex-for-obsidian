...
export default class ClaudianPlugin extends Plugin {
  ...
  async onload() {
    ...
    this.addRibbonIcon('bot', 'Open Codex for Obsidian', () => {
      this.activateView();
    });
  ...
  async loadSettings() {
    ...
    this.conversations = allMetadata.map(meta => {
      ...
      return {
        ...
        externalAttachmentPaths: meta.externalAttachmentPaths,
        ...
        for (const tab of affectedTabs) {
          ...
          try {
            const externalContextPaths = mergeOrderedExternalContextPaths(
              tab.ui.externalFileAttachmentManager?.getAttachedPaths() ?? [],
