...
import { formatExternalReferences } from '../../../utils/externalReferences';
...
export function encodeCodexTurn(request: ChatTurnRequest): PreparedChatTurn {
  ...
  if (
    ...
    && !request.text.includes('<external_reference ')
  ) {
    sections.push(`\n${formatExternalReferences(request.externalContextPaths)}`);
  }
