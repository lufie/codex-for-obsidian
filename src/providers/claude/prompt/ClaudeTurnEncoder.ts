...
import { appendExternalReferences } from '../../../utils/externalReferences';
...
export function encodeClaudeTurn(
  ...
  if (!isCompact) {
    ...
    if (
      ...
      && request.externalContextPaths.length > 0
      && !persistedContent.includes('<external_reference ')
    ) {
      persistedContent = appendExternalReferences(persistedContent, request.externalContextPaths);
    }
  }
