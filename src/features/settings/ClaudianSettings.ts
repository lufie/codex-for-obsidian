...
export class ClaudianSettingTab extends PluginSettingTab {
  ...
  display(): void {
    ...
    if (providerTabs.length === 1 && providerTabs[0] === 'codex') {
      const content = containerEl.createDiv({
        cls: 'claudian-settings-tab-content claudian-settings-tab-content--active claudian-settings-single-page',
      });
      const advancedSection = createAdvancedSettingsSection(
        t('settings.advancedSection.title'),
        t('settings.advancedSection.singlePageDesc'),
      );
      const advancedContainer = advancedSection.contentEl;
      const providerContext = {
        ...
        renderCustomContextLimits: (target: HTMLElement, providerId?: ProviderId) =>
          this.renderCustomContextLimits(target, providerId),
        advancedContainer,
      };

      ProviderWorkspaceRegistry.getSettingsTabRenderer('codex')?.render(content, providerContext);
      this.renderGeneralTab(content, { advancedContainer });
      content.appendChild(advancedSection.detailsEl);
      return;
    }
  ...
  private renderGeneralTab(container: HTMLElement, options: GeneralTabOptions = {}): void {
    const ownAdvancedSection = options.advancedContainer
      ? null
      : createAdvancedSettingsSection(
        t('settings.advancedSection.title'),
        t('settings.advancedSection.generalDesc'),
      );
    const advancedContainer = options.advancedContainer ?? ownAdvancedSection!.contentEl;
    ...
    const maxTabsSetting = new Setting(advancedContainer)
      .setName(t('settings.maxTabs.name'))
      .setDesc(t('settings.maxTabs.desc'));

    const maxTabsWarningEl = advancedContainer.createDiv({ cls: 'claudian-max-tabs-warning' });
    ...
    const hotkeyGrid = advancedContainer.createDiv({ cls: 'claudian-hotkey-grid' });
    ...
    if (ownAdvancedSection) {
      container.appendChild(ownAdvancedSection.detailsEl);
    }
  }
