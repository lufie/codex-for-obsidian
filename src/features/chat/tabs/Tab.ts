...
import { mergeOrderedExternalContextPaths } from '../../../utils/externalContext';
import { extractOrderedInlineReferencePaths } from '../../../utils/inlineReferences';
...
import { ExternalFileAttachmentManager } from '../ui/ExternalFileAttachmentManager';
import { FileContextManager } from '../ui/FileContext';
import { ImageContextManager } from '../ui/ImageContext';
import { InlineReferenceComposerManager } from '../ui/InlineReferenceComposerManager';
...
export function createTab(options: TabCreateOptions): TabData {
  ...
  const tab: TabData = {
    ...
    ui: {
      ...
      externalFileAttachmentManager: null,
      inlineReferenceComposerManager: null,
      modelSelector: null,
...
function autoResizeTextarea(textarea: HTMLTextAreaElement): void {
  ...
  // Empty input should never force the wrapper to grow.
  // This avoids height jumps during tab/session creation while layout is still settling.
  if (!textarea.value) {
  ...
  const overlayEl = textarea
    .closest('.claudian-input-wrapper')
    ?.querySelector('.claudian-inline-reference-overlay') as HTMLElement | null;
  let measuredScrollHeight = textarea.scrollHeight;
  if (overlayEl) {
    const prevHeight = overlayEl.style.height;
    overlayEl.style.height = 'auto';
    measuredScrollHeight = overlayEl.scrollHeight;
    overlayEl.style.height = prevHeight;
  }
  const contentHeight = Math.min(measuredScrollHeight, maxHeight);
...
function buildTabDOM(contentEl: HTMLElement): TabDOMElements {
  ...
  const pinnedChipsEl = messagesEl.createDiv({ cls: 'claudian-pinned-wikilinks' });
  ...
  const inputEl = inputWrapper.createEl('textarea', {
    cls: 'claudian-input',
    attr: {
      placeholder: 'Ask Codex to help with your vault...',
      rows: '3',
      dir: 'auto',
      spellcheck: 'false',
      autocorrect: 'off',
...
export async function initializeTabService(
  ...
  try {
    ...
    if (conversation) {
      ...
      const externalContextPaths = mergeOrderedExternalContextPaths(
        hasMessages ? (conversation.externalAttachmentPaths || []) : [],
        hasMessages
...
function initializeContextManagers(tab: TabData, plugin: ClaudianPlugin): void {
  ...
  tab.ui.inlineReferenceComposerManager = new InlineReferenceComposerManager(
    dom.inputEl,
    dom.inputWrapper,
    {
      onChange: () => {
        const inlinePaths = extractOrderedInlineReferencePaths(dom.inputEl.value);
        tab.ui.externalFileAttachmentManager?.retainAttachedPaths(inlinePaths);
        autoResizeTextarea(dom.inputEl);
      },
    },
  );

  // External file attachment manager - drag/drop and file picker for external files
  tab.ui.externalFileAttachmentManager = new ExternalFileAttachmentManager(
    dom.inputContainerEl,
    dom.inputEl,
    dom.contextRowEl,
    {
...
function initializeInputToolbar(
  ...
  // Attach external file button (paperclip icon) — positioned at end of toolbar
  const attachBtn = inputToolbar.createDiv({
    cls: 'claudian-external-file-btn',
    attr: { 'aria-label': t('chat.externalFile.attachButton') },
  });
  attachBtn.innerHTML = `
  ...
  attachBtn.addEventListener('click', () => {
    tab.ui.externalFileAttachmentManager?.openFilePicker();
  });
  ...
  tab.controllers.conversationController = new ConversationController(
    {
      ...
      getExternalFileAttachmentManager: () => ui.externalFileAttachmentManager,
...
export async function destroyTab(tab: TabData): Promise<void> {
  ...
  tab.ui.externalFileAttachmentManager?.destroy();
  tab.ui.inlineReferenceComposerManager?.destroy();
