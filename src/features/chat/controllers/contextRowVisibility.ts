export function updateContextRowHasContent(contextRowEl: HTMLElement): void {
  ...
  const externalFilesChips = contextRowEl.querySelector('.claudian-external-files-chips') as HTMLElement | null;
  ...
  const hasExternalFileChips = externalFilesChips?.style.display === 'flex';
