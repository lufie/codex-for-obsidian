...
export class ConversationController {
  ...
  async createNew(options: { force?: boolean } = {}): Promise<void> {
    ...
    try {
      ...
      this.deps.getExternalFileAttachmentManager?.()?.clearAttachments();
  ...
  async save(updateLastResponse = false, options?: SaveOptions): Promise<void> {
    ...
    const externalAttachmentPaths = this.deps.getExternalFileAttachmentManager?.()?.getAttachedPaths() ?? [];
    ...
    const updates: Partial<Conversation> = {
      ...
      externalAttachmentPaths: externalAttachmentPaths.length > 0 ? externalAttachmentPaths : undefined,
  ...
  private restoreConversation(
    ...
    const externalContextPaths = this.getRuntimeExternalContextPaths(conversation, hasMessages);
    ...
    void this.restoreExternalAttachmentPaths(conversation.externalAttachmentPaths, !hasMessages);
  ...
  private async restoreExternalAttachmentPaths(
    savedPaths: string[] | undefined,
    isEmptySession: boolean
  ): Promise<void> {
    const attachmentManager = this.deps.getExternalFileAttachmentManager?.();
    ...
    await attachmentManager.setAttachedPaths(savedPaths || []);
  }

  private getRuntimeExternalContextPaths(
    conversation: Conversation,
    hasMessages: boolean
  ): string[] {
    const externalAttachmentPaths = hasMessages ? (conversation.externalAttachmentPaths || []) : [];
    const externalContextPaths = hasMessages
      ? (conversation.externalContextPaths || [])
      : (this.deps.plugin.settings.persistentExternalContextPaths || []);

    return mergeOrderedExternalContextPaths(
      externalAttachmentPaths,
  ...
  getGreeting(): string {
    ...
    const getTimeGreetings = (): string[] => {
      if (hour >= 5 && hour < 12) {
        return [personalize('Good morning'), 'Ready to work with Codex?'];
