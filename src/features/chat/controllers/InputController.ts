...
import {
  expandInlineReferencesToXml,
  extractOrderedInlineReferencePaths,
  formatInlineReferencesForDisplay,
...
import type { ExternalFileAttachmentManager } from '../ui/ExternalFileAttachmentManager';
...
export interface InputControllerDeps {
  ...
  getExternalFileAttachmentManager?: () => ExternalFileAttachmentManager | null;
...
export class InputController {
  ...
  async sendMessage(options?: {
    ...
    const externalFileAttachmentManager = this.deps.getExternalFileAttachmentManager?.();
    ...
    if (builtInCmd) {
      if (shouldUseInput) {
        ...
        externalFileAttachmentManager?.clearAttachments?.();
      }
    ...
    try {
      ...
      if (!wasInvalidated && state.streamGeneration === streamGeneration) {
        ...
        if (didCancelThisTurn && !state.pendingNewSessionPlan) {
          await streamController.appendText('\n\n<span class="claudian-interrupted">Interrupted</span> <span class="claudian-interrupted-hint">· What should Codex do instead?</span>');
        }
        streamController.hideThinkingIndicator();
        state.isStreaming = false;
        state.cancelRequested = false;
  ...
  private buildTurnSubmission(options: {
    ...
    const inlineReferencePaths = extractOrderedInlineReferencePaths(options.content);

    // Explicit drag-and-drop references take priority over broad external context roots.
    const uniqueExternalPaths = mergeOrderedExternalContextPaths(
      inlineReferencePaths,
      externalContextPaths ?? [],
    );

    const isCompact = /^\/compact(\s|$)/i.test(options.content);
    const displayContent = formatInlineReferencesForDisplay(options.content);
    const contentWithInlineReferences = expandInlineReferencesToXml(options.content);
    const transformedText = !isCompact && fileContextManager
      ? fileContextManager.transformContextMentions(contentWithInlineReferences)
      : contentWithInlineReferences;
    ...
    return {
      ...
      turnRequest: {
        ...
        externalContextPaths: uniqueExternalPaths.length > 0
          ? uniqueExternalPaths
          : undefined,
  ...
  private handleProviderUserMessageStart(
    ...
    const displayContent = expected.displayContent;
  ...
  cancelStreaming(): void {
    ...
    // Finalize any active thinking block so it gets saved to contentBlocks
    streamController.finalizeCurrentThinkingBlock(this.activeStreamingAssistantMessage ?? undefined);
    streamController.hideThinkingIndicator();
  }
