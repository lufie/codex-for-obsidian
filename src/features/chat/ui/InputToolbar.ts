...
export class ThinkingBudgetSelector {
  ...
  private render() {
    ...
    effortLabel.setText(t('chat.toolbar.effortLabel'));
  ...
  private renderBudgetGears() {
    ...
    for (const budget of [...options].reverse()) {
      ...
      gearEl.setAttribute('title', tokens > 0
        ? t('chat.toolbar.tokenCount', { count: tokens.toLocaleString() })
        : t('common.disabled'));
...
export class PermissionToggle {
  ...
  private render() {
    ...
    this.addOption(toggleConfig.inactiveValue, toggleConfig.inactiveLabel, t('settings.permission.tooltipConfirm'));
    this.addOption(toggleConfig.activeValue, toggleConfig.activeLabel, t('settings.permission.tooltipAuto'));
    if (toggleConfig.planValue) {
      this.addOption(toggleConfig.planValue, toggleConfig.planLabel ?? t('settings.permission.plan'), t('settings.permission.tooltipPlan'));
    }
  ...
  updateDisplay() {
    ...
    for (const [value, optionEl] of this.optionEls) {
      const isPlanOption = planValue === value;
      optionEl.style.display = isPlanOption && !canShowPlan ? 'none' : '';
      optionEl.toggleClass('is-active', value === mode);
    }
  }

  private addOption(value: string, label: string, title: string): void {
    const optionEl = this.container.createEl('button', {
      cls: 'claudian-permission-option',
      text: label,
      type: 'button',
    });
    optionEl.setAttribute('title', title);
    optionEl.addEventListener('click', async () => {
      await this.callbacks.onPermissionModeChange(value);
      this.updateDisplay();
    });
    this.optionEls.set(value, optionEl);
  }
...
export class ServiceTierToggle {
  ...
  updateDisplay() {
    ...
    this.container.setAttribute('title', t('chat.toolbar.fastModeToggle'));
  }
...
export class ExternalContextSelector {
  ...
  addExternalContext(pathInput: string): AddExternalContextResult {
    ...
    if (!trimmed) {
      return { success: false, error: t('chat.externalContext.noPath') };
    }
    ...
    if (!path.isAbsolute(normalizedPath)) {
      return { success: false, error: t('chat.externalContext.pathMustBeAbsolute') };
    }
    ...
    if (isDuplicatePath(normalizedPath, this.externalContextPaths)) {
      return { success: false, error: t('chat.externalContext.duplicate') };
    }
  ...
  private async openFolderPicker() {
    try {
      ...
      const result = await remote.dialog.showOpenDialog({
        ...
        title: t('chat.externalContext.pickerTitle'),
      });
      ...
      new Notice(t('chat.externalContext.pickerError'), 5000);
    }
  }
  ...
  private formatConflictMessage(newPath: string, conflict: { path: string; type: 'parent' | 'child' }): string {
    ...
    return conflict.type === 'parent'
      ? t('chat.externalContext.conflictInside', { newPath: shortNew, existingPath: shortExisting })
      : t('chat.externalContext.conflictContains', { newPath: shortNew, existingPath: shortExisting });
  }
  ...
  private renderDropdown() {
    ...
    headerEl.setText(t('chat.externalContext.title'));
    ...
    if (this.externalContextPaths.length === 0) {
      ...
      emptyEl.setText(t('chat.externalContext.empty'));
    } else {
      for (const pathStr of this.externalContextPaths) {
        ...
        lockBtn.setAttribute('title', isPersistent
          ? t('chat.externalContext.makeSessionOnly')
          : t('chat.externalContext.makePersistent'));
        ...
        setIcon(removeBtn, 'x');
        removeBtn.setAttribute('title', t('chat.externalContext.removePath'));
  ...
  updateDisplay() {
    ...
    if (count > 0) {
      ...
      this.iconEl.setAttribute('title', t('chat.externalContext.countTitle', { count }));
      ...
      this.iconEl.setAttribute('title', t('chat.externalContext.addTitle'));
      this.badgeEl.removeClass('visible');
    }
  }
}
...
export class McpServerSelector {
  ...
  private renderDropdown() {
    ...
    headerEl.setText(t('chat.mcp.title'));
    ...
    if (servers.length === 0) {
      ...
      emptyEl.setText(allServers.length === 0
        ? t('chat.mcp.noneConfigured')
        : t('chat.mcp.allDisabled'));
      return;
    }
  ...
  private renderServerItem(listEl: HTMLElement, server: ManagedMcpServer) {
    ...
    if (server.contextSaving) {
      ...
      csEl.setText('@');
      csEl.setAttribute('title', t('chat.mcp.contextSaving', { name: server.name }));
    }
  ...
  updateDisplay() {
    ...
    if (count > 0) {
      this.iconEl.addClass('active');
      this.iconEl.setAttribute('title', t('chat.mcp.countTitle', { count }));
      ...
      this.iconEl.removeClass('active');
      this.iconEl.setAttribute('title', t('chat.mcp.enableTitle'));
      this.badgeEl.removeClass('visible');
    }
  }
}
